...
function compareRows(left: FetchRowData, right: FetchRowData, sortKey: FetchSortKey): number {
  if (sortKey === "revalidate") {
    ...
    const rightValue = typeof right.fetchCall.revalidate === "number" ? right.fetchCall.revalidate : Number.MAX_SAFE_INTEGER;
...
export default function FetchesViewClient(): React.JSX.Element | null {
  ...
  const filteredRows = useMemo(() => {
    return rows
      ...
      .filter((row) => {
        ...
        if (cacheFilter === "unset") return row.fetchCall.cache === undefined;
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap gap-3 rounded-lg border border-[#333] bg-[#111] p-4">
        <select
          value={routeTypeFilter}
          onChange={(event: React.ChangeEvent<HTMLSelectElement>): void => {
            setRouteTypeFilter(event.target.value as RouteTypeFilter);
          }}
          className="rounded-md border border-[#333] bg-[#0a0a0a] px-3 py-1.5 text-sm text-white outline-none focus:border-[#FFC000]"
        >
        ...
        <label className="inline-flex items-center gap-2 rounded-md border border-[#333] bg-[#0a0a0a] px-3 py-1.5 text-sm text-gray-300">
          <input
            type="checkbox"
            ...
            className="h-4 w-4 rounded border-[#333] bg-[#111] text-[#FFC000]"
          />
      ...
      <div className="overflow-hidden rounded-lg border border-[#333] bg-[#111]">
        <div className="grid grid-cols-[2.2fr_1.5fr_1fr_1fr_1.5fr_0.6fr] gap-4 border-b border-[#333] px-4 py-3 text-[11px] font-medium uppercase tracking-wide text-gray-500">
        ...
        <div className="max-h-[calc(100vh-14rem)] overflow-y-auto">
          {groupedRows.map(([routeId, routeRows]) => (
            <section key={routeId}>
              <div className="sticky top-0 z-10 border-b border-[#333] bg-[#0a0a0a] px-4 py-2">
                <p className="font-mono text-xs text-white">{routeRows[0].route.path}</p>
                <p className="text-sm text-gray-400">{routeId}</p>
              </div>

              {routeRows.map((row) => (
                <div
                  key={row.fetchCall.id}
                  className="grid grid-cols-[2.2fr_1.5fr_1fr_1fr_1.5fr_0.6fr] gap-4 border-b border-[#333] px-4 py-3 text-sm text-gray-300"
                >
                  <a
                    href={getVsCodeHref(row.fetchCall.sourceFile)}
                    className="inline-flex min-w-0 items-center gap-2 font-mono text-xs text-gray-200 transition-colors hover:text-[#FFC000]"
                  >
                    <span className="truncate">{row.fetchCall.sourceFile}</span>
                    <ExternalLink className="h-3.5 w-3.5 shrink-0" />
                  </a>
                  <span className="truncate font-mono text-xs text-gray-300">{row.fetchCall.url ?? "dynamic url"}</span>
