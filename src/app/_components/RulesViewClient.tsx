...
export default function RulesViewClient(): React.JSX.Element | null {
  ...
  const filteredAntiPatterns = useMemo(() => {
    ...
    if (antiPatternFilter === "all") {
      return graph.antiPatterns;
    }
  ...
  return (
    <section className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-amber-500/20 bg-zinc-900 p-4">
          <p className="text-[11px] font-medium uppercase tracking-wide text-amber-400">Warnings</p>
          <p className="mt-2 text-2xl font-semibold text-zinc-50">{warningCount}</p>
        </div>
        <div className="rounded-lg border border-red-500/20 bg-zinc-900 p-4">
          ...
          <p className="mt-2 text-2xl font-semibold text-zinc-50">{errorCount}</p>
        </div>
      </div>

      <div className="flex gap-2 rounded-lg border border-zinc-800 bg-zinc-900 p-2">
        {(["all", "warning", "error"] as const).map((filter) => (
          <button
            key={filter}
            type="button"
            onClick={(): void => {
              setAntiPatternFilter(filter);
            }}
            className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${
              antiPatternFilter === filter ? "bg-zinc-800 text-zinc-50" : "text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200"
            }`}
          >
      ...
      <div className="space-y-4">
        {groupedAntiPatterns.map(([ruleName, antiPatterns]) => (
          <section key={ruleName} className="rounded-lg border border-zinc-800 bg-zinc-900">
            <div className="border-b border-zinc-800 px-4 py-3">
              <p className="font-mono text-xs text-zinc-50">{ruleName}</p>
            </div>

            <div className="space-y-3 p-4">
              {antiPatterns.map((antiPattern) => (
                <article key={antiPattern.id} className="rounded-lg border border-zinc-800 bg-zinc-950 p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={getSeverityBadgeClassName(antiPattern.severity)}>{antiPattern.severity}</span>
                    <span className="rounded border border-zinc-700 bg-zinc-800 px-1.5 py-0.5 text-[11px] font-medium text-zinc-300">
                      {antiPattern.rule}
                    </span>
                  </div>

                  <p className="mt-3 text-sm text-zinc-200">{antiPattern.message}</p>
                  <a
                    href={getVsCodeHref(antiPattern.sourceFile, antiPattern.line)}
                    className="mt-3 inline-flex font-mono text-xs text-zinc-400 transition-colors hover:text-[#FFC000]"
                  >
